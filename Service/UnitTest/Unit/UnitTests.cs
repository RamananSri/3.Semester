...
namespace UnitTest
{
    ...
    public class UnitTests
    {
        ...
        {
            double price = 1;
            var sameDate = _bCtrl.CalcPrice(DateTime.Now.Date, DateTime.Now.Date, price);
            Assert.AreEqual(price,sameDate);

            try
            {
                _bCtrl.CalcPrice(DateTime.Now.AddDays(2).Date, DateTime.Now, price);
            }
            catch (FaultException e)
            {
                Assert.AreEqual(e.Message, "Date error");
            }
        }
